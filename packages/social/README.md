...
Tests are located alongside source files in `__tests__` directories, one per
module (e.g. `src/posts/__tests__/mutations.test.ts` tests `src/posts/mutations.ts`).
Database-backed modules (mutations, queries, feeds) mock `@scilent-one/db`
with `vi.mock`, following the pattern in `src/reposts/__tests__/mutations.test.ts`.

### What's Tested

Every module in `src/` has a corresponding test file, covering happy paths,
validation errors, and authorization/not-found edge cases for
database-backed mutations and queries. Run `pnpm test:coverage` for the
current numbers; as of this writing the package is at ~99% statement/line
...
3. **Per-package coverage PR comment** - Posts/updates a sticky comment with a coverage table (see [Per-Package Coverage Comment](#per-package-coverage-comment) below)
4. **Codecov integration** - Uploads coverage and posts PR comments (See [Coverage Reporting](#coverage-reporting))

## Per-Package Coverage Comment

In addition to Codecov, the CI workflow posts (and keeps updated) a sticky PR
comment with a simple per-package coverage table, generated by
`scripts/generate-coverage-comment.mjs` from each package's
`coverage/coverage-summary.json`.

Coverage reporting is rolled out one package at a time. `@scilent-one/social`
is the first package wired up; the rest of the packages listed in the table
(`scilent-ui`, `ui`, `db`, `harmony-engine`) show `TBA` until they're added to
the `PACKAGES` list in that script. To wire up a new package:

1. Ensure its `vitest.config.ts` includes `'json-summary'` in
   `coverage.reporter` (so `coverage-summary.json` is produced).
2. Add it to the `--filter` list in the "Run tests with coverage" step of
   `.github/workflows/test.yml` (if it isn't already included).
3. Add a `summaryPath` entry for it in `scripts/generate-coverage-comment.mjs`.
